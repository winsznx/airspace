...
import {AirspacePortfolioV1Unsafe} from "./AirspacePortfolio.v1.0.0.UNSAFE.sol";
...
///      points at it.
contract AirspacePortfolioFactoryV1Unsafe {
    error DeployFailed();
    ...
    function createPortfolio(address owner_, bytes32 salt) external returns (address portfolio) {
        ...
        portfolio = _clone(s);
        AirspacePortfolioV1Unsafe(payable(portfolio)).initialize(owner_, module, outcomeToken, collateral);
