...
for (let i=0;i<12;i++){
  ...
  const { data: jobs } = await db.from("reconciliation_jobs").select("status").limit(500);
  const c={}; for (const j of jobs??[]) c[j.status]=(c[j.status]??0)+1;
  const { count: res } = await db.from("reservations").select("*",{count:"exact",head:true});
  console.log(`domainUsage ${usage}  reserved ${reserved}  reservations ${res}  jobs ${JSON.stringify(c)}`);
